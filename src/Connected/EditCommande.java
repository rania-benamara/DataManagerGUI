...
public class EditCommande extends JDialog {
	...
	public static EditCommande current;    /* pour sauter d'une fen�tre � l'autre */
	
	
	class MyEditTableModel extends DefaultTableModel {
		private String columnNames []={"Cli_Id", "P_Id", "C_Quantite"};
...
//	}
	...
	public EditCommande() {
		            ...
		            if (myValidate()) {
		                try {
		                    PreparedStatement stmt = EmpGUI.current.con.prepareStatement(
		                            "UPDATE commande SET C_Quantite = ?  WHERE Cli_Id = ? AND P_Id=?"
		                    );
		                ...
		                EditCommande.current.dispose(); 
		            } 
		        } else { // insertion
		            if (myValidate()) {
		                try {
		                    PreparedStatement stmt = EmpGUI.current.con.prepareStatement(
		                            "INSERT INTO commande (Cli_Id, P_Id, C_Quantite) VALUES (?, ?, ?)"
		                    );
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				EditCommande.current.dispose();   // pour fermer la fen�tre. 
			}
		});
	...
	private boolean myValidate() {	
		...
		if(!isUpdate) {      // si c'est une insertion
			...
			if(!isValidateIDProduit(table.getValueAt(0, 1))) {    // si le nouveau id est invalide (n�gatif)
				JOptionPane.showMessageDialog(null, "Id invalide");
				return false;				                  // dans ce cas, la fonction fini ici 
			}
			/*
			...
			try {
				ResultSet rs = EmpGUI.current.stmt.executeQuery( 
						"SELECT * FROM commande WHERE Cli_Id =  '"+ table.getValueAt(0, 0)+"'" );
	...
	static boolean isValidateIDProduit(Object object) {
