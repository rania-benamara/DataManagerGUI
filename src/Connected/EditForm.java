...
public class EditForm extends JDialog {
	...
	class MyEditTableModel extends DefaultTableModel {
		private String columnNames []={"Cli_Id", "Cli_PNom", "Cli_Nom"};
		            ...
		            if (myValidate()) {
		                try {
		                    PreparedStatement stmt = EmpGUI.current.con.prepareStatement(
		                            "UPDATE client SET Cli_PNom = ?, Cli_Nom = ? WHERE Cli_Id = ?"
		                    );
		                ...
		                EditForm.current.dispose();  // fermer la fenêtre
		            } 
		        } else { // insertion
		            if (myValidate()) {
		                try {
		                    PreparedStatement stmt = EmpGUI.current.con.prepareStatement(
		                            "INSERT INTO client (Cli_Id, Cli_PNom, Cli_Nom) VALUES (?, ?, ?)"
		                    );
		...
		if(!isUpdate) {      // si c'est une insertion
			...
			try {
				ResultSet rs = EmpGUI.current.stmt.executeQuery( 
						"SELECT * FROM client WHERE Cli_Id =  '"+ table.getValueAt(0, 0)+"'" );
